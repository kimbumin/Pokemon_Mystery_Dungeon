...
{
    {
        tiniWoods = ImageManager::GetInstance()->AddImage(
            "TiniWoods3", L"Image/SceneImage/TiniWoods3.bmp", 525, 600,
            SAMPLE_TILE_X, SAMPLE_TILE_Y, true, RGB(255, 0, 255));

        sea1 = ImageManager::GetInstance()->AddImage(
            "Sea1", L"Image/SceneImage/Sea1.bmp", 525, 600, 18, SAMPLE_TILE_Y,
            true, RGB(255, 0, 255));

        sea2 = ImageManager::GetInstance()->AddImage(
            "Sea3", L"Image/SceneImage/Sea3.bmp", 450, 600, 18, SAMPLE_TILE_Y,
            true, RGB(255, 0, 255));

        magma1 = ImageManager::GetInstance()->AddImage(
            "MagmaCave1", L"Image/SceneImage/MagmaCave1.bmp", 525, 600, 21,
            SAMPLE_TILE_Y, true, RGB(255, 0, 255));

        ice1 = ImageManager::GetInstance()->AddImage(
            "IceMountain1", L"Image/SceneImage/IceMountain1.bmp", 675, 600, 27,
            SAMPLE_TILE_Y, true, RGB(255, 0, 255));

        ice2 = ImageManager::GetInstance()->AddImage(
            "IceMountain2", L"Image/SceneImage/IceMountain2.bmp", 675, 600, 27,
            SAMPLE_TILE_Y, true, RGB(255, 0, 255));
        //--
        magma2 = ImageManager::GetInstance()->AddImage(
            "MagmaCave2", L"Image/SceneImage/MagmaCave2.bmp", 750, 600, 30,
            SAMPLE_TILE_Y, true, RGB(255, 0, 255));
        tiniMeadow = ImageManager::GetInstance()->AddImage(
            "TinyMeadow", L"Image/SceneImage/TinyMeadow.bmp", 750, 600, 30,
            SAMPLE_TILE_Y, true, RGB(255, 0, 255));
        //--

        stairs = ImageManager::GetInstance()->AddImage(
            "stairs", L"Image/SceneImage/stairs.bmp", 24, 24, 1, 1, true,
            RGB(255, 0, 255));
    }
...
void Map::Render(HDC hdc)
{
    ...
    {
        tileImage->FrameRender(hdc, pt.x * TILE_SIZE, pt.y * TILE_SIZE, 13, 1,
                               0, 1);
    }
    // 이동가능 타일
    for (const POINT& pt : floorTiles)
    {
        ...
        {
            tileImage->FrameRender(hdc, pt.x * TILE_SIZE, pt.y * TILE_SIZE,
                                   indexPos.first, indexPos.second, 0, 1);
        }
        else
        {
            if (indexPos == WEEDS)
...
{  // 과정 여기다 할까 생각중
    for (int y = 0; y < TILE_Y; ++y)
    {
        for (int x = 0; x < TILE_X; ++x)
        {
            RECT rc = {x * TILE_SIZE, y * TILE_SIZE, (x + 1) * TILE_SIZE,
                       (y + 1) * TILE_SIZE};
            HBRUSH brush =
                (tiles[y][x] == TILE_FLOOR or tiles[y][x] == TILE_PATH)
...
void Map::MiniMapRender(HDC hdc, int drawingX, int drawingY)
{
    ...
    for (int y = 0; y < TILE_Y; ++y)
    {
        for (int x = 0; x < TILE_X; ++x)
        {
            RECT rc = {drawingX + x * MINIMAP_TILE_SIZE,
            ...
            switch (tiles[y][x])
            {
                ...
                case TILE_PATH:
                    brush =
                        CreateSolidBrush(RGB(150, 150, 255));  // 연한 파란색
                    break;
                case TILE_WALL:
    ...
    RECT stairRc = {drawingX + stairPos.x * MINIMAP_TILE_SIZE,
...
{
    return tiles[y][x] == TileType::TILE_PATH ||
           tiles[y][x] == TileType::TILE_FLOOR;
}

bool Map::MatchPattern(const WildPattern& pattern,
                       const vector<int>& current) const
{
...
{
    tileIndex.clear();
    tileIndex.resize(TILE_Y,
                     vector<pair<int, int>>(TILE_X, {4, 1}));  // 기본 벽 타일
