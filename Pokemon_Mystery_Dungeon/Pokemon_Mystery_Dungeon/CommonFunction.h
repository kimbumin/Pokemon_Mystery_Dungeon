...
{
    RECT rc{x - (width / 2), y - (height / 2), x + (width / 2),
            y + (height / 2)};
    return rc;
}
...
// 가운데 기준
inline void RenderRectAtCenter(HDC hdc, int centerX, int centerY, int width,
                               int height)
{
...
// 가운데 기준
inline void RenderEllipseAtCenter(HDC hdc, int centerX, int centerY, int width,
                                  int height)
{
...
{
    if (rc1.right < rc2.left || rc1.left > rc2.right || rc1.top > rc2.bottom ||
        rc1.bottom < rc2.top)
    {
        return false;
    }
