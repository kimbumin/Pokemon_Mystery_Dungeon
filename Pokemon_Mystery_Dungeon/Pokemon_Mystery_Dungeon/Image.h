...
public:
    ...
    // 파일로부터 이미지를 로드하는 함수
    HRESULT Init(const wchar_t* filePath, int width, int height, int maxFrameX,
                 int maxFrameY, bool isTransparent = FALSE,
    ...
    void FrameRender(HDC hdc, int destX, int destY, int frameX, int frameY,
                     bool isFlip = false, bool isCenter = true);
    void FrameRenderCamera(HDC hdc, int destX, int destY, int frameX,
    ...
    {
        return imageInfo->frameWidth;
    }
    inline int GetFrameHeight()
    {
        return imageInfo->frameHeight;
    }
};
