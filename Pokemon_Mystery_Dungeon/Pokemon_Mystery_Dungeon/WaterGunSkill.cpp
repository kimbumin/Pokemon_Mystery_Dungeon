...
WaterGunSkill::WaterGunSkill(const SkillData& skillData)
{
    data = skillData;
    direction = 0;
    pos = {0, 0};
    ...
    image = ImageManager::GetInstance()->AddImage(
        "WaterGunSkill", TEXT("Image/SkillImage/WaterGunSkill.bmp"), 1000, 50, 20,
        1, true, RGB(255, 0, 255));
}

HRESULT WaterGunSkill::Init()
{
    return S_OK;
}

void WaterGunSkill::Release()
{
}

void WaterGunSkill::Update()
{
    if (isActive)
    {
...
void WaterGunSkill::Render(HDC hdc)
{
    if (isActive && image)
    {
...
void WaterGunSkill::Use(PokemonBase* owner)
{
...
shared_ptr<ISkill> WaterGunSkill::Clone()
{
    return make_shared<WaterGunSkill>(*this);  // 스킬을 복제하여 반환
}

void WaterGunSkill::Reset()
{
    isActive = false;
    frameCount = 0;
    elapsedTime = 0.0f;
