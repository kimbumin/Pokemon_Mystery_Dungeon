...
HRESULT TilemapTool::Init()
{
    ...
    sampleTile = ImageManager::GetInstance()->AddImage(
        "smapleTile", L"Image/SceneImage/TiniWoods3.bmp", 525, 600,
...
void TilemapTool::Render(HDC hdc)
{
    ...
    {
        sampleTile->FrameRender(hdc, tileInfo[i].rc.left, tileInfo[i].rc.top,
                                tileInfo[i].frameX, tileInfo[i].frameY, false,
                                false);
    }
    ...
    // 선택된 타일
    sampleTile->FrameRender(hdc, TILEMAPTOOL_X - sampleTile->GetWidth(),
                            sampleTile->GetHeight() + 100, selectedTile.x,
                            selectedTile.y, false, false);
    ...
    sampleTile->Render(hdc, sampleTileStartX, 0);
    {
        ...
        Rectangle(hdc, sampleTileStartX + selectedTile.x * TILE_SELECT_SIZE,
...
{
    // 파일 저장
    HANDLE hFile = CreateFile(L"TileMapData.dat", GENERIC_WRITE, 0, NULL,
    ...
    CloseHandle(hFile);
    MessageBox(g_hWnd, TEXT("타일맵이 성공적으로 저장되었습니다."),
               TEXT("완료"), MB_OK);
}
...
{
    // 파일 로드
    HANDLE hFile = CreateFile(L"TileMapData.dat", GENERIC_READ, 0, NULL,
    ...
    CloseHandle(hFile);
    MessageBox(g_hWnd, TEXT("타일맵이 성공적으로 로드되었습니다."),
               TEXT("완료"), MB_OK);
}
