...
HRESULT SquareScene::Init()
{
    ...
    yellowFlower = ImageManager::GetInstance()->AddImage(
        "³ë¶õ²É", L"Image/SceneImage/YellowFlower.bmp", 33, 144, 1, 6, 0,
        RGB(184, 240, 120));

    redFlower = ImageManager::GetInstance()->AddImage(
        "ºÓÀº²É", L"Image/SceneImage/RedFlower.bmp", 33, 144, 1, 6, 0,
        RGB(184, 240, 120));
...
void SquareScene::Update()
{
    ...
    if (KeyManager::GetInstance()->IsOnceKeyDown(VK_F5))
    {
        ...
        SceneManager::GetInstance()->AddLoadingScene("·Îµù¾À",
                                                     new LoadingScene());
